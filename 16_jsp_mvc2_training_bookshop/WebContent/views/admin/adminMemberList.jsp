...
<script src="views/js/jquery-3.6.1.min.js"></script>
<script>
	
	function realDelete(memberId) {
		
		var answer = confirm("정말 삭제함?");
		if(answer) {
			var href = "adminDeleteMember?memberId=" + memberId;
			location.href = href;
		}
	...
	$().ready(function(){
		...
		$("#cnt").change(function(){
			alert("${searchWord }")
			if ($("#cnt").val() == "10") {
				location.href="adminMemberList?sort=${sort }&searchWord=${searchWord }&viewCnt=10&page=1";
			} else if ($("#cnt").val() == "7") {
				location.href="adminMemberList?sort=${sort }&searchWord=${searchWord }&viewCnt=7&page=1";
			} else if ($("#cnt").val() == "5") {
				location.href="adminMemberList?sort=${sort }&searchWord=${searchWord }&viewCnt=5&page=1";
			}
                        ...
                        <ul>
                            <li><a href="#">Management(구현)</a>
                                <ul class="dropdown">
                                    ...
                                    <li><a href="adminMemberList">Member Management</a></li>
                                </ul>
                            </li>
                        </ul>
                    </nav>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb__links">
                        <a href="adminMemberList"><i class="fa fa-home"></i> Home</a>
                        <span>Admin Member List</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <select id="cnt">
                        ...
                        <table>
                            ...
                            <tbody>
                            	<c:forEach var="memberDTO" items="${memberDTOList }">
	                                <tr>
	                                	<td><a href="adminMemberDetail?memberId=${memberDTO.memberId }">${memberDTO.memberId }</a></td>
	                                	<td>${memberDTO.memberNm }</td>
	                                    <td><strong>${memberDTO.point } P</strong></td>
	                                    <td>${memberDTO.joinDt }</td>
	                                    <td><input type="button" value="수정" onclick="location.href='adminModifyMember?memberId=${memberDTO.memberId}'">
	                                    	<input type="button" value="삭제" onclick="realDelete(${memberDTO.memberId})">
	                                    </td>
	                                </tr>
                        ...
                        <form action="adminMemberList" method="get">
                        	<select name="sort">
                        		...
                        		<option value="point">~이상의 포인트 보유</option>
                        		<option value="year">~이후 가입(년)</option>
                        	</select>
